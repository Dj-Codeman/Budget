...
<div>
<form style="height:100%; width:90%; margin: auto;" method="post" action="income.php" enctype="multipart/form-data">
...
<div class="input-group" style=" position: relative; width:50%;  margin:auto;">
    ...
    <textarea class="input-group" type="text" style="height:5%;" name="Deposit_desc" ></textarea>
  </div>
<!-- income category -->
...
<div class="input-group" style="width:50%; margin:auto;">
    <label>Deposit Category</label>
		<select style="width:100%; height:70%;" name="Deposit_catg" id="Deposit_catg">
...
?>
  ...
  <div class="input-group">
    <button style="margin-left:25%;" type="submit" class="btn-login" name="Deposit">Add Transactions</button>
  </div>
...
if (isset($_POST['Deposit'])) {
  $title     = mysqli_real_escape_string($db, $_POST['Deposit_name']);
  $desc      = mysqli_real_escape_string($db, $_POST['Deposit_desc']);
  $acnt_name = mysqli_real_escape_string($db, $_POST['Account_name']);
  $amnt      = mysqli_real_escape_string($db, $_POST['Deposit_amount']);
  $catg      = mysqli_real_escape_string($db, $_POST['Deposit_catg']);
	$flag      = "name";
  // array_push($errors, "NULL");

  deposit($title,$desc,$acnt_name,$amnt,$catg,$flag);
	update_accounts();

  if ($status == '0') {
  echo '<script type="text/javascript">';
  echo "alert('Deposit Submitted ".$amnt." !');";
