...
function add_bill($title,$desc,$acnt_name,$amnt_1,$day,$catg,$split) {
...
$stmt = mysqli_prepare($db, "INSERT INTO Budget.Bills ( Bill_owner, uid, Bill_title, Bill_desc, Bill_amnt, Bill_acnt, Bill_date, Bill_stus, Bill_chrg_date, Bill_split, Bill_catg, Bill_sec)
VALUES(?,?,?,?,?,?,?,?,?,?,?)" );

//cheap work-around
$zero = 0;

mysqli_stmt_bind_param($stmt, "ssssssssssss", $user, $uid, $title, $desc, $amnt, $number, $date, $zero, $day, $split, $catg, $section );
...
if ($exempt == '1'){
$confy = 100;
if ( between($balance, $confy, 200.00)){
$color = "color:red";
} elseif ($balance <= 0) {
$color = "color:gray";
...
$button .= "<a href=\"https://budget.dwcloud.tk/account_delete.php?item=$name \">";
...
echo "<td>$catg</td>";
$button = "<td style=\"text-align:center; width:10%; \">";
$button .= "<a href=\"https://budget.dwcloud.tk/bill_delete.php?item=$name \">";
...
$button .= "<a href=\"https://budget.dwcloud.tk/category_delete.php?item=$name \">";
...
function clear_slate(){
  global $db, $user;
  //wiping chart data
  $query = "DELETE FROM Budget.Trans_record WHERE owner = '$user'";
  mysqli_query($db, $query);
  // Whiping chart data
  $query = "TRUNCATE TABLE Budget_$user";
  mysqli_query($db, $query);
  ...
  //updating privious logs to status 2
  $set_status = "UPDATE Budget.Income SET Income_stus = '2' WHERE Income_stus = '1' AND Income_owner = '$user' ";
  mysqli_query($db, $set_status);
}
...
while($row = mysqli_fetch_assoc($result)) {
  ...
  $Account_num = $row['Account_number'];
  $Account_cur_balance = $row['Account_cur_balance'];
  $Account_old_balance = $row['Account_int_balance'];
  // set cur balance to init bbalance
  // set every trans
  $zero_account = "UPDATE Budget.Accounts SET Account_int_balance = '$Account_cur_balance' WHERE Account_number = '$Account_num' ";
...
$budget = $array[$i]["Budget"];
  ...
  $array[$i]["Budget"] = $array[$i]["Budget"];

}

# TOM FOOLERY TO BE FIXED LATER
...
$paid = $data['Paid'];
// # bug fix
// # if the paid variable is null
// # the bill update function will fail
// if (is_null($paid)) {
//   $num = '0.00';
//   $paid = convert_number($num);
// }
// # same fix for Legnth
// if (is_null($legnth)) {
//   $legnth = '14';
...
# getting the old bill price to revert the budget
$catg = sanatize_category(mysqli_get_var("SELECT Bill_catg FROM Budget.Bills WHERE uid = '$bill_id'"));
$created = convert_number(mysqli_get_var("SELECT Created FROM Budget.Budget_$user WHERE Category = '$catg'"));
$old_price = convert_number(mysqli_get_var("SELECT Bill_amnt FROM Budget.Bills WHERE uid = '$bill_id'"));

$price = convert_number($price);
// var_dump($price);

$tmp_price = convert_number($created - $old_price);
$new_price = convert_number($tmp_price + $price);

$query4 = "UPDATE Budget.Bills SET Bill_amnt = '$price' WHERE uid = '$bill_id'";
$query5 = "UPDATE Budget.Bills SET Bill_stus = '0' WHERE uid = '$bill_id'";
var_dump($query4);
var_dump($query5);
mysqli_query($db, $query4);
mysqli_query($db, $query5);
$query6 = "UPDATE Budget.Target_$user SET Amount = '$new_price' WHERE Category = '$catg' ";
...
function section($day) {
  ...
  if (between($day, '16', '22') == true) { return '3'; }
  if (between($day, '23', '31') == true) { return '4'; }
}
...
// Finding gathering category array
$Categories = category_array();
foreach($Categories as $catg){
  $catg = sanatize_category($catg);
  // Selecting already added category amount
  $catg_total = convert_number(mysqli_get_var("SELECT Amount from Target_$user where Category = '$catg'"));

  $query = "SELECT * FROM Budget.Bills WHERE Bill_owner = '$user' AND Bill_sec = '$section1' AND Bill_stus <> '3' OR Bill_owner = '$user' AND Bill_sec = '$section2' AND Bill_stus <> '3' OR Bill_owner = '$user' AND Bill_split = '1' AND Bill_stus <> '3' ";
  $result = mysqli_query($db, $query);
  while($row = mysqli_fetch_assoc($result)) {
    $bill_amnt = $row['Bill_amnt'];
    $bill_uid = $row['uid'];
    $bill_name = $row['Bill_title'];
    $bill_split = $row['Bill_split'];
    $bill_catg = $row['Bill_catg'];
    if ( $bill_split == '0') { $bill_tmp = $bill_amnt; }
    if ( $bill_split == '1') { $bill_tmp = $bill_amnt / 2; }
    $total = number_format($bill_tmp, 2, '.', ',');
    $catg_total += $total;
    $query = "UPDATE Budget.Bills SET Bill_stus = '3' WHERE uid = '$bill_uid' ";
    mysqli_query($db, $query);

  }
  $number = convert_number($catg_total);
  // echo "$number";
  $query = "UPDATE Budget.Target_$user SET Amount = '$number' WHERE Category = '$catg' ";
  mysqli_query($db, $query);
}
reset_budget();
}
